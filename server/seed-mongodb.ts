  ...
  try {
    ...
    if (userCount === 0) {
      ...
      if (finalProject && finalExam && deptMeeting) {
        ...
        const notificationResult = await collections.notifications.insertMany(notifications);
        console.log(`Created ${Object.keys(notificationResult.insertedIds).length} notifications`);
      }
